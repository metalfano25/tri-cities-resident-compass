  ...
  return ["geneva", "batavia", "st-charles"].flatMap((communityId) =>
    items
      .filter((item) => {
        ...
        return Number.isFinite(start) && Number.isFinite(end) && end >= now - 86_400_000 && start <= now + UPCOMING_EVENT_WINDOW_MS;
      })
...
export async function getLiveDataPayload(): Promise<LiveDataPayload> {
  ...
  const notices = balancedNotices(uniqueBy(results.flatMap((result) => result.notices), (item) => item.canonicalUrl || item.id));
  const events = balancedEvents(uniqueBy(results.flatMap((result) => result.events), (item) => `${item.canonicalUrl}|${item.startAt ?? item.dateLabel}`));
